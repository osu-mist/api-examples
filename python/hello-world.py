...
def getAccessToken(url, client_id, client_secret):
	...
	curl.setopt(curl.POST, 1)                        # CURLOPT_POST in PHP
	curl.setopt(curl.POSTFIELDS, post_data)          # CURLOPT_POSTFIELDS in PHP
...
def getLocations(url, access_token, params):
	...
	headers = {"Authorization: Bearer " + access_token: None}
...
access_token_url = base_url + config_data["hostname"]
